...
export async function POST(request: Request) {
  ...
  if (!externalRes.ok) {
    ...
    return NextResponse.json(
      {
        error: `SentinelOne a repondu ${externalRes.status} ${externalRes.statusText}`,
  ...
  if (!payload || !Array.isArray(payload.data)) {
    return NextResponse.json(
      { error: "Reponse SentinelOne inattendue (champ data manquant)" },
      { status: 422 },
    );
  }
  ...
  const items = candidates.map((c) => ({
    ...
    action:
      existingRefs.has(c.externalRef) || existingRefs.has(c.legacyExternalRef)
        ? ("skip" as const)
        : ("create" as const),
  }));
  ...
  return NextResponse.json({
    total: items.length,
    createCount: items.filter((x) => x.action === "create").length,
    skipCount: items.filter((x) => x.action === "skip").length,
